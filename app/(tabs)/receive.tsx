...
import { View, Text, StyleSheet, TouchableOpacity, Share, Clipboard, Image, Alert, ScrollView } from 'react-native';
...
const BLOCKCHAINS_LIST = [
  ...
  },
  {
    ...
    type: 'evm',
    isEvmGroup: true,
    note: 'Supports Ethereum, BNB Chain, Polygon, Arbitrum, Base, Optimism, Avalanche, Fantom, and all ERC-20 tokens.'
  },
  {
    id: 'ton',
    name: 'TON',
...
export default function ReceiveScreen() {
  const { accountNumber, isDarkMode } = useUserStore();
        ...
        <View style={styles.body}>
          ...
          <View style={styles.addressesStack}>
            {BLOCKCHAINS_LIST.map((chain) => {
              ...
              const displayedAddress = `${chain.address.slice(0, 10)}...${chain.address.slice(-10)}`;
                    ...
                    <View style={styles.blockchainLeft}>
                      ...
                      <View style={styles.logosRowContainer}>
                        {chain.logos.map((logoSource, idx) => {
                          const isLocal = typeof logoSource !== 'string';
                          const resolvedSource = isLocal ? logoSource : { uri: logoSource };
                          return (
                            <Image
                              key={isLocal ? idx : (logoSource as string)}
                    ...
                    <View style={styles.expandedContent}>
                      ...
                      <View style={styles.fullAddressContainer}>
                        ...
                        <View style={styles.fullAddressRow}>
                          ...
                          <TouchableOpacity
                            onPress={() => handleCopy(chain.address, `${chain.name} address`)}
                            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                          >
