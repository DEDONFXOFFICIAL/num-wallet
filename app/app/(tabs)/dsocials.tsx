...
export default function DsocialsScreen() {
  ...
  useEffect(() => {
    const fetchSocials = async () => {
      try {
        ...
        console.warn('Failed to load curated socials:', e);
      } finally {
        setLoading(false);
      }
    };
    ...
    // Setup real-time listener for curated items database changes
    const channel = supabase
      .channel('realtime_dsocials')
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'curated_items',
        filter: 'category=eq.dsocials'
      }, () => {
        fetchSocials();
      })
      .subscribe();
    ...
    <SafeAreaView style={bgStyle} edges={['top']}>
      ...
      <View style={[styles.header, borderStyle, { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing[2] }}>
          <Ionicons name="people" size={20} color={Colors.brand.bright} />
          <Text style={[styles.headerTitle, isDarkMode ? styles.textWhite : styles.textLightPrimary]}>Decentralized Socials Portal</Text>
        </View>
        <TouchableOpacity
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* Curated Socials List */}
        <View style={styles.lobbyWrapper}>
          <Text style={[styles.sectionTitle, !isDarkMode && styles.textLightSecondary]}>
            Curated Social Feed & Apps
          </Text>

          {loading ? (
            <View style={[styles.curatorCard, !isDarkMode && styles.curatorCardLight]}>
              <ActivityIndicator size="small" color={Colors.brand.bright} />
              <Text style={[styles.curatorTitle, isDarkMode ? styles.textWhite : styles.textLightPrimary]}>Loading Social Platforms...</Text>
            </View>
          ) : items.length === 0 ? (
            /* Curator Empty State */
            <View style={[styles.curatorCard, !isDarkMode && styles.curatorCardLight]}>
              <View style={styles.iconBox}>
                <Ionicons name="chatbubbles-outline" size={32} color={isDarkMode ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)'} />
              </View>
              <Text style={[styles.curatorTitle, isDarkMode ? styles.textWhite : styles.textLightPrimary]}>No Social Apps Listed</Text>
              <Text style={[styles.curatorDesc, isDarkMode ? styles.textMuted : styles.textLightMuted]}>
                Decentralized Socials Portal
              </Text>
            </View>
          ) : (
            /* Curated Apps Grid/List */
