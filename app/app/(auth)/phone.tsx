...
const COUNTRY_CODES = [
// ... (COUNTRY_CODES remain unchanged)
...
export default function PhoneScreen() {
  ...
  const Alert = {
    alert: (title: string, message?: string, buttons?: any[]) => {
      ...
      if (lowerTitle.includes('success') || lowerTitle.includes('copied') || lowerTitle.includes('found')) {
        icon = 'check-circle';
        iconColor = '#10B981';
      } else if (lowerTitle.includes('failed') || lowerTitle.includes('error') || lowerTitle.includes('incorrect') || lowerTitle.includes('invalid') || lowerTitle.includes('not found')) {
        icon = 'alert-triangle';
        iconColor = '#EF4444';
      }
  ...
  const lastOtpVerifiedAt = useUserStore((s) => s.lastOtpVerifiedAt);

  const isPhoneValid = phone.length >= selected.digits;
  const isPasscodeValid = mode === 'login' ? passcode.length === 6 : true;
  const isValid = isPhoneValid && isPasscodeValid;
  ...
  const handleContinue = async () => {
    ...
    // 2. Handle automatic transitions/alerts for incorrect flow selection
    if (isAlreadyRegistered && mode !== 'login') {
      setLoading(false);
      Alert.alert(
        'Account Found',
        'An account with this phone number already exists. Switching you to Sign In mode.',
        [
          {
            text: 'Continue to Sign In',
            onPress: () => {
              // Redirect to Sign In mode so the passcode input shows up for them to enter
              router.replace({
                pathname: '/(auth)/phone',
                params: { mode: 'login' }
              });
      ...
      return;
    } else if (!isAlreadyRegistered && mode === 'login') {
      setLoading(false);
      Alert.alert(
        'Account Not Found',
        'No registered account was found for this number. Switching you to Sign Up/Registration mode.',
        [
          {
    ...
    await sendOtpAndNavigate(fullPhone, cleanNum, mode);
  };

  const sendOtpAndNavigate = async (fullPhone: string, cleanNum: string, targetMode: string | undefined) => {
    // 1. If we are logging in, check if we verified OTP recently (within 30 days) to bypass SMS
    if (targetMode === 'login') {
      const lastOtpVerifiedAt = useUserStore.getState().lastOtpVerifiedAt;
      const ONE_MONTH_MS = 30 * 24 * 60 * 60 * 1000; // 30 days
      const needsOtp = !lastOtpVerifiedAt || (Date.now() - lastOtpVerifiedAt > ONE_MONTH_MS);

      if (!needsOtp) {
        // Bypass OTP! Since passcode is already validated, route directly to main app dashboard
        useUserStore.getState().setAccountNumber(cleanNum);
        ...
        try {
          ...
          console.warn('Failed to sync profile from Supabase on bypass:', e);
        }
    ...
    // 2. Otherwise send SMS OTP code to confirm number ownership (e.g. signup or monthly check)
    setLoading(true);
    const result = await SmsService.sendOtp(fullPhone);
    setLoading(false);
    ...
    if (result.success) {
      ...
      Alert.alert('Error', result.error || 'Failed to deliver verification code.');
    }
  };
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        ...
        >
          ...
          {/* Passcode input (Visible only in Sign In / Login mode) */}
          {mode === 'login' && (
            <View style={styles.passcodeInputSection}>
              <View style={styles.passcodeInputWrap}>
                <Feather name="lock" size={18} color={Colors.text.muted} style={styles.passcodeIcon} />
                <TextInput
