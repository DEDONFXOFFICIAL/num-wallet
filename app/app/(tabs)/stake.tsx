...
export default function StakeScreen() {
  const { isDarkMode, addTransaction } = useUserStore();
  ...
  const Alert = {
    ...
    prompt: (title: string, message?: string, buttons?: any[]) => {
          ...
          onPress: (address: string | undefined) => {
            if (address && address.trim().length > 10) {
              ...
              addTransaction({
                ...
                chain: 'Ethereum',
                status: 'Success',
                txHash: address ? address : '0x' + Math.random().toString(16).substring(2, 18)
              });
    ...
    Alert.alert(
      ...
      [{ 
        ...
        onPress: () => {
          ...
          addTransaction({
            type: 'Yield Stake',
            fromSymbol: selectedNetwork.token,
            toSymbol: selectedValidator.name,
