  ...
  useEffect(() => {
    const fetchPredictions = async () => {
      try {
        ...
        console.warn('Failed to load curated predictions:', e);
      } finally {
        setLoading(false);
      }
    };
    ...
    // Setup real-time listener for curated predictions database changes
    ...
    <SafeAreaView style={bgStyle} edges={['top']}>
      ...
      <View style={[styles.header, borderStyle, { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing[2] }}>
          <Ionicons name="stats-chart" size={20} color={Colors.brand.bright} />
          <Text style={[styles.headerTitle, isDarkMode ? styles.textWhite : styles.textLightPrimary]}>Decentralized Predictions Portal</Text>
        </View>
        <TouchableOpacity
        ...
        <View style={styles.lobbyWrapper}>
          ...
          {loading ? (
            <View style={[styles.curatorCard, !isDarkMode && styles.curatorCardLight]}>
              ...
              <Text style={[styles.curatorTitle, isDarkMode ? styles.textWhite : styles.textLightPrimary]}>Loading Markets...</Text>
            </View>
          ) : items.length === 0 ? (
            /* Curator Empty State */
            <View style={[styles.curatorCard, !isDarkMode && styles.curatorCardLight]}>
              <View style={styles.gameIconBox}>
                <Ionicons name="stats-chart-outline" size={32} color={isDarkMode ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)'} />
              </View>
              <Text style={[styles.curatorTitle, isDarkMode ? styles.textWhite : styles.textLightPrimary]}>No Prediction Markets Listed</Text>
